...
const safe_redirect_path = (value: string | null): string => {
    if (!value || !value.startsWith('/') || value.startsWith('//')) return '/';
    if (value.startsWith('/sign-in') || value.startsWith('/auth/callback')) return '/';
    return value;
};
...
export default function SignInPage() {
    ...
    const redirectPath = safe_redirect_path(searchParams.get('redirect'));

    // If already signed in, bounce to dashboard
    useEffect(() => {
        if (auth_storage.get_access_token()) navigate(redirectPath, { replace: true });
    }, [navigate, redirectPath]);

    const handleLogin = (action: 'signin' | 'signup') => {
        try {
            start_google_login({ flow: 'web_dashboard', redirect: redirectPath }, action);
        } catch {
    ...
    const handleEmailSignIn = async (e: React.FormEvent) => {
        ...
        try {
            ...
            window.location.href = isAdmin && redirectPath === '/' ? '/admin' : redirectPath;
        } catch (err: any) {
